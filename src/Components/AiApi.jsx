...
export const summarizeText = async (pdfFile, isCreativeMode = false) => {
  try {
    ...
    console.log('Uploading file:', {
      ...
      type: pdfFile.type,
      mode: isCreativeMode ? 'creative' : 'deterministic'
    });

    const endpoint = isCreativeMode ? '/pdf/creativesummarize' : '/pdf/summarize';
...
export const answerQuestion = async (file, question, isCreativeMode = false) => {
  try {
    const formData = new FormData();
    ...
    const endpoint = isCreativeMode ? '/pdf/creativeanswer' : '/pdf/answer';
    const response = await api.post(endpoint, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
